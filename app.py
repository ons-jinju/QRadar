...
st.markdown("""
<style>
html,body,[class*="css"]{font-family:'Malgun Gothic','Segoe UI',sans-serif}

/* ── 메트릭 카드 ── */
[data-testid="stMetric"]{
    background:#ffffff;border:1px solid #e2e6f2;border-radius:10px;
    padding:14px 16px;box-shadow:0 1px 4px rgba(26,39,68,0.07);
}
[data-testid="stMetricValue"]{font-size:1.55rem;font-weight:800;color:#1a2744}
[data-testid="stMetricLabel"]{font-size:0.76rem;font-weight:700;color:#5c6690}
[data-testid="stMetricDelta"]{font-size:0.8rem;font-weight:700}

/* ── 사이드바 ── */
[data-testid="stSidebar"]{background:linear-gradient(180deg,#16213e 0%,#1a2744 100%)}
[data-testid="stSidebar"] *{color:#e7ebfa!important}
[data-testid="stSidebar"] h2,[data-testid="stSidebar"] h3{color:#ffffff!important}
[data-testid="stSidebar"] hr{border-color:#33406e!important}

/* ── 파일 업로더 (또렷하게) ── */
[data-testid="stFileUploader"] section{
    background:#22335c;border:2px dashed #5b7ae0;border-radius:10px;
    padding:18px;transition:all .15s ease;
}
[data-testid="stFileUploader"] section:hover{background:#2b3e70;border-color:#8aa0f2}
[data-testid="stFileUploader"] section *{color:#eef1fc!important;opacity:1!important}
[data-testid="stFileUploader"] section svg{fill:#9db1f0!important}
[data-testid="stFileUploader"] small{color:#b7c2ec!important}
[data-testid="stFileUploader"] button{
    background:#3a5fcc!important;color:#ffffff!important;border:1px solid #4c70e0!important;
    border-radius:6px!important;font-weight:700!important;padding:6px 14px!important;
}
[data-testid="stFileUploader"] button:hover{background:#4c70e0!important}
[data-testid="stFileUploaderFile"]{
    background:#22335c;border-radius:8px;border:1px solid #3a4a78;margin-top:6px;
}
[data-testid="stFileUploaderFile"] *{color:#eef1fc!important}

/* ── 메인 헤더 ── */
.main-header{background:linear-gradient(120deg,#1a2744 0%,#2b4585 55%,#3a5fcc 100%);
             color:#fff;padding:18px 24px;border-radius:10px;margin-bottom:18px;
             box-shadow:0 4px 14px rgba(26,39,68,0.2)}
.main-header h1{font-size:1.4rem;font-weight:800;margin:0;letter-spacing:-.2px}
.main-header p{font-size:0.8rem;color:#d3ddf7;margin:4px 0 0}

/* ── 섹션 타이틀 ── */
.sec-title{font-size:0.87rem;font-weight:800;color:#22305a;background:#edf1fb;
           border-left:4px solid #3a5fcc;padding:7px 10px;border-radius:5px;margin:8px 0 12px}

/* ── 배지 ── */
.file-type-badge{display:inline-block;padding:4px 12px;border-radius:14px;
                 font-size:11px;font-weight:800;margin-right:6px;
                 background:#edf1fb;color:#22305a}

/* ── 탭 ── */
div[data-testid="stTabs"] button{font-size:0.85rem;font-weight:600;color:#4a5478}
div[data-testid="stTabs"] button[aria-selected="true"]{color:#3a5fcc!important;font-weight:800}
div[data-testid="stTabs"] div[data-baseweb="tab-highlight"]{background-color:#3a5fcc!important}

/* ── 데이터프레임 ── */
[data-testid="stDataFrame"]{border:1px solid #e2e6f2;border-radius:8px;overflow:hidden}

/* ── 버튼 ── */
[data-testid="stDownloadButton"] button, [data-testid="baseButton-secondary"]{
    background:#3a5fcc!important;color:#fff!important;border:none!important;
    border-radius:6px!important;font-weight:700!important;
}
[data-testid="stDownloadButton"] button:hover{background:#4c70e0!important}
...
# ────────────────────────────────────────────
# Plotly 공통 스타일 (Streamlit 다크모드와 무관하게 항상 또렷하게)
# ────────────────────────────────────────────
def apply_theme(fig):
    """모든 차트에 밝고 또렷한 공통 테마를 강제 적용"""
    fig.update_layout(
        paper_bgcolor='#ffffff',
        plot_bgcolor='#f7f9fc',
        font=dict(family="Malgun Gothic, Segoe UI, sans-serif", color='#1a2744', size=12),
        title_font=dict(size=13, color='#1a2744', family="Malgun Gothic, Segoe UI, sans-serif"),
        legend=dict(font=dict(color='#3a4470', size=10)),
        xaxis=dict(gridcolor='#e6e9f4', zerolinecolor='#e6e9f4', linecolor='#d3d8ec',
                    tickfont=dict(color='#3a4470')),
        yaxis=dict(gridcolor='#e6e9f4', zerolinecolor='#e6e9f4', linecolor='#d3d8ec',
                    tickfont=dict(color='#3a4470')),
        hoverlabel=dict(bgcolor='#1a2744', font_color='#ffffff', font_size=12),
    )
    return fig

def show_chart(fig):
    """테마 강제 적용 후 출력 (Streamlit 자동 다크/라이트 재색칠 방지)"""
    apply_theme(fig)
...
def render_quality_kpi(df, filters):
    ...
    with tabs[0]:
        ...
        if '_일자' in df.columns and '_시' in df.columns:
            ...
            fig.add_hline(y=filters['prb_thresh'], line_dash='dash', line_color='#d92626',
        ...
        if name_col in df.columns:
            ...
            fig2 = px.bar(sp, x=name_col, y='pmprbuseddlavg',
                          ...
                          color_continuous_scale=[[0,'#2fa876'],[0.3,'#f0b429'],[0.6,'#e6772e'],[1,'#c81e1e']],
                          title='국소별 DL PRB 최대 (상위 20)')
            fig2.add_hline(y=filters['prb_thresh'], line_dash='dash', line_color='#d92626')
        ...
        with c2:
            if 'cd_rate' in df.columns and name_col in df.columns:
                ...
                fig3 = px.bar(cd, x=name_col, y='cd_rate', title='국소별 CD율 평균 (상위 15)',
                              color='cd_rate',
                              color_continuous_scale=[[0,'#2fa876'],[0.5,'#f0b429'],[1,'#c81e1e']])
        ...
        with c1:
            if sinr_cols and name_col in df.columns:
                ...
                fig4 = px.bar(sinr_df.head(20), x=name_col, y=sinr_cols[0],
                              ...
                              color=sinr_cols[0],
                              color_continuous_scale=[[0,'#c81e1e'],[0.5,'#f0b429'],[1,'#2fa876']])
    ...
    with tabs[3]:
        ...
        if '_시' in df.columns and name_col in df.columns:
            ...
            colorscale = [
                [0,                               '#eaf6f0'],
                [max(filters['prb_thresh']/100-0.001, 0), '#eaf6f0'],
                [filters['prb_thresh']/100,        '#f7d5ae'],
                [0.50,  '#e6772e'],
                [0.70,  '#c81e1e'],
                [1.0,   '#7a0000'],
            ]
            fig6 = go.Figure(go.Heatmap(
                ...
                textfont=dict(size=8, color='#1a2744'),
    ...
    with tabs[0]:
        ...
        if name_col in df.columns and cfi_cols:
            ...
            for qc in cfi_cols:
                ...
                fig.add_vline(x=bad_thresh, line_dash='dash', line_color='#d92626',
                              annotation_text=f'불량기준 {bad_thresh}')
                fig.update_layout(height=250, margin=dict(l=5,r=5,t=40,b=20), showlegend=False)
                show_chart(fig)
    ...
    with tabs[1]:
        if '_일자' in df.columns:
            ...
            fig2 = px.line(g, x='_일자', y=cfi_cols, markers=True, title='일자별 CFI 트렌드',
                            color_discrete_sequence=['#3a5fcc','#2fa876'])
            fig2.add_hline(y=filters.get('cfi_thresh',70), line_dash='dash', line_color='#d92626')
    ...
    with tabs[0]:
        if grade_cols:
            ...
            fig = px.pie(values=total.values, names=total.index,
                         title='CQ 등급 전체 분포',
                         color_discrete_sequence=['#2f6fed','#2fa876','#f0b429','#e6772e','#9b4fdb'])
    ...
    with tabs[1]:
        if '_일자' in df.columns and 'CQ 1등급 비율' in df.columns:
            ...
            fig3 = px.line(g, x='_일자', y='CQ 1등급 비율', markers=True,
                           ...
                           color_discrete_sequence=['#3a5fcc'])
            fig3.add_hline(y=filters.get('cq_thresh',85), line_dash='dash', line_color='#d92626',
    ...
    with tabs[2]:
        ...
        if bad_cols and name_col in df.columns:
            ...
            fig4.update_layout(height=320, margin=dict(l=5,r=5,t=40,b=80),
                                xaxis_tickangle=-35, legend=dict(font=dict(size=10)))
            show_chart(fig4)
...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown("<div style='font-size:10px;color:#8b96c4'>진주품질개선팀 · 내부용</div>",
                unsafe_allow_html=True)
...
# ── 전체 파일 공통 날짜 범위 필터 (파일 1개/여러 개 모두 적용)
all_dates = [df['_date'].dropna() for _, df, _ in file_results if '_date' in df.columns]
all_dates = pd.concat(all_dates) if all_dates else pd.Series(dtype='datetime64[ns]')

if not all_dates.empty:
    min_d, max_d = all_dates.min().date(), all_dates.max().date()
    st.markdown('<div class="sec-title">📅 날짜 필터 (전체 파일 공통)</div>', unsafe_allow_html=True)
    dc1, dc2 = st.columns(2)
    with dc1:
        date_start = st.date_input("시작일", value=min_d, min_value=min_d, max_value=max_d)
    with dc2:
        date_end = st.date_input("종료일", value=max_d, min_value=min_d, max_value=max_d)
    if date_start > date_end:
        st.error("시작일이 종료일보다 늦을 수 없습니다.")
        st.stop()

    new_results = []
    for fname, df, ftype in file_results:
        if '_date' in df.columns:
            df = df[(df['_date'].dt.date >= date_start) & (df['_date'].dt.date <= date_end)]
        new_results.append((fname, df, ftype))
    file_results = new_results

    if min_d == max_d:
        st.caption(f"ℹ️ 업로드된 데이터는 {min_d.strftime('%Y-%m-%d')} 하루 분량만 포함되어 있습니다. 여러 날짜를 비교하려면 다른 날짜의 파일도 함께 업로드하세요.")

st.markdown("---")


def render_one(fname, df, ftype, key_prefix):
    sig = FILE_SIGNATURES.get(ftype, {})
    ...
    if sites:
        sel_sites = st.multiselect("🏢 국소/장비 (전체=선택 안함)", sites, placeholder="전체",
...
if len(file_results) == 1:
    ...
    render_one(fname, df, ftype, key_prefix="single")
    ...
    for i, (fname, df, ftype) in enumerate(file_results):
        with tabs[i]:
            render_one(fname, df, ftype, key_prefix=f"tab{i}")
